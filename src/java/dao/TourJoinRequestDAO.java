...
public class TourJoinRequestDAO extends DBContext {
    ...
    public int updateTour(TourJoinRequest tour) throws Exception {
        ...
        try {
            con = getConnection();
            ps = con.prepareStatement("UPDATE tour_join_request SET team_id = ?, tour_id = ?, status = ? WHERE request_id = ?;");
    ...
    public TourJoinRequest GetTourJoinRequestByID(int id) throws Exception {
        try {
            String query = "SELECT * FROM tour_join_request where request_id = ? ";
            con = getConnection();
            ps = con.prepareStatement(query);
            ps.setInt(1, id);
            rs = ps.executeQuery();
    ...
    public List<TourJoinRequest> GetTourJoinByTeamID(int team_id) throws Exception {
        try {
            String query = "SELECT * FROM tour_join_request where team_id = ?";
            con = getConnection();
            ps = con.prepareStatement(query);
            ps.setInt(1, team_id);
            rs = ps.executeQuery();
